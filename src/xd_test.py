...
def test(model, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, DNP_use, args, device, logger=None):
    ...
    msg1 = "AUC1: {:.6f}  AP1: {:.6f}".format(ROC1, AP1)
    msg2 = "AUC2: {:.6f}  AP2: {:.6f}".format(ROC2, AP2)
    if logger:
        logger.info(msg1)
        logger.info(msg2)
    else:
    ...
    averageMAP = 0
    for i in range(5):
        msg = 'mAP@{0:.1f} ={1:.4f}%'.format(iou[i], dmap[i])
        if logger:
            logger.info(msg)
        else:
            print(msg)
    ...
    msg_avg = 'average MAP: {:.4f}'.format(averageMAP)
    if logger:
        logger.info(msg_avg)
    else:
        print(msg_avg)

    # 返回所有需要的指标，以便主函数获取最优AP2
