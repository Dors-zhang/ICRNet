...
def test(model, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, DNP_use, device, args, logger=None):
    ...
    averageMAP = 0
    for i in range(5):
        msg = 'mAP@{0:.1f} ={1:.2f}%'.format(iou[i], dmap[i])
        if logger:
            logger.info(msg)
        else:
            print(msg)
    ...
    msg_avg = 'average MAP: {:.2f}'.format(averageMAP)
    if logger:
        logger.info(msg_avg)
    else:
        print(msg_avg)
